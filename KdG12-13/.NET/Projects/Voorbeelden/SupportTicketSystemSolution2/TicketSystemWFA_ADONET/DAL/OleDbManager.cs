...
namespace TicketSystem.DAL
{
    ...
    {
        private OleDbDataAdapter daTickets;

        public OleDbManager()
        {
            daTickets = new OleDbDataAdapter();
            string selectSql = "SELECT Id, AccountId, DateOpened, State FROM tblTickets";
            OleDbCommand selectCmd = new OleDbCommand(selectSql, GetConnection());
            daTickets.SelectCommand = selectCmd;
            OleDbCommandBuilder cb = new OleDbCommandBuilder(daTickets);
        }

        public DataSet GetTicketsDataSet()
        {
            DataSet dsTickets = new DataSet();
            daTickets.Fill(dsTickets, "tblTickets");
            return dsTickets;
        }

        public void UpdateTicketsDataSet(DataSet dsTickets)
        {
            daTickets.Update(dsTickets.Tables["tblTickets"]);
            // daTickets.Update(dsTickets, "tblTickets");
        }
        ...
        private OleDbConnection GetConnection()
        {
            ...
            conn.ConnectionString = ConfigurationManager.ConnectionStrings["TicketSystemDB.accdb"].ConnectionString;
            return conn;
        }
